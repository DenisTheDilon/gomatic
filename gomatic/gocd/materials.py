...
class PackageMaterial(CommonEqualityMixin):
    ...
    def of(repository_name=None, package_name=None, configurator=None):
        ...
        package = config.find('./repositories/repository[@name="%s"]/packages/package[@name="%s"]' % (repository_name, package_name))

        if package is None:
            raise RuntimeError('Package: "%s" not found in the repository: "%s"' % (package_name, repository_name))
